...
function renderCode(value, render) {
  return (
    <Box sx={{ display: "flex", flexDirection: "column" }}>
      <Typography variant="body1" sx={{ mb:.5 }}>{value}</Typography>
      <Typography variant="caption" color="text.disabled" fontWeight="600">{render.external_code}</Typography>
    </Box>
  )
}
...
export default function TableComponent({ addModalOpen, addModalId }) {
  ...
  const bodis = new TableRowsMixin(tableData, {
    id: (value) => (
      <IconButton onClick={() => {addModalId(value);addModalOpen();}} color="primary">
        <EditIcon />
      </IconButton>
    ),
    ...
    <>
      <Grid container spacing={2}>
        <Grid item xs={6} md={6} lg={6} sx={{ display: "flex", alignItems: "center" }}>
          <Typography variant="h5" fontWeight="600">Articoli di magazzino</Typography>
        </Grid>
