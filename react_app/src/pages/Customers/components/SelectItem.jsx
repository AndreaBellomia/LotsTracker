...
export default function ArticlesListModal({ modalState: modaleState, tableChoices: tableChoices, type: type }) {
    ...
    useEffect(() => {
        try {
            ...
            new ItemsApi().getWarehouseItemsAvailable(pageSelected, search, " ").then((response) => {
                                    ...
                                    <TableRow key={index}>
                                        <TableCell onClick={() => submitSelect(obj)}>
                                            <Box>
                                                <Typography variant="body1">
                                                    {obj.item_type.description ? obj.item_type.description : "--"}
                                                </Typography>
                                                <Typography
                                                    variant="body2"
                                                    color="text.disabled"
                                        ...
                                        <TableCell onClick={() => submitSelect(obj)} sx={{ textAlign:  "end" }}>
                                            <Typography variant="body2" fontWeight="bold">
                                                {obj.item_type.external_code ? obj.item_type.external_code : "--"}
                                            </Typography>
                                        </TableCell>
                                    </TableRow>
                                ))}
